...
description: This data has been provided by NASA's Goddard Institute for Space Studies
draft: false
image: pic08.jpg
keywords: ""
slug: climate
...
This data has been provided by NASA's Goddard Institute for Space Studies in the *Combined Land-Surface Air and Sea-Surface Water Temperature Anomalies*s](https://data.giss.nasa.gov/gistemp).
...
Notice that, when using this function, I added two options: `skip` and `na`.
...
1. Select the year and the twelve month variables from the `weather` dataset. I do not need the others (J-D, D-N, DJF, etc.) for this assignment. Hint: use `select()` function.
...
My dataframe should have three variables now, one each for 
...
Let us plot the data using a time-series scatter plot, and add a trendline. To do that, I first need to create a new variable called `date` in order to ensure that the `delta` values are plot chronologically. 
...
Is the effect of increasing temperature more pronounced in some months? Use `facet_wrap()` to produce a separate scatter plot for each month, again with a smoothing line. My chart should human-readable labels; that is, each month should be labeled "Jan", "Feb", "Mar" (full or abbreviated month names are fine), not `1`, `2`, `3`. 
...
I removed data from before 1800 and before using `filter`. Then, I used the `mutate` function to create a new variable `interval` which contains information on which period each observation belongs to. I can assign the different periods using `case_when()`.
...
Now that I have the `interval` variable, I can create a density plot to study the distribution of monthly deviations (`delta`), grouped by the different time periods I am interested in. Set `fill` to `interval` to group and colour the data by different time periods.
...
So far, I have been working with monthly anomalies. However, I might be interested in average annual anomalies. I can do this by using `group_by()` and `summarise()`, followed by a scatter plot to display the result. 
...
My task was to construct a confidence interval for the average annual delta since 2011, both using a formula and using a bootstrap simulation with the `infer` package. Recall that the dataframe `comparison` has already grouped temperature anomalies according to time intervals; I  are only interested in what is happening  between 2011-present.
